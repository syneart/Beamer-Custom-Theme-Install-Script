...
set str=%PATH: =/%&set MiKTeXPath=nul
FOR %%b in ("%str%") DO (
...
set MiKTeXPath=%MiKTeXPath:miktex\bin\=%
set MiKTeXPath=%MiKTeXPath:/= %
...
initexmf --update-fndb 1>nul 2>nul
if !errorlevel!==1 (
...
FOR /R %~1 %%c in (%~2) DO (
	...
	copy "%%c" "%MiKTeXPath%tex\latex\beamer\base\!themesFile!" 1>nul 2>nul
)
